      ...
      <div className="ftabs">
        {files.map((f) => (
          <button
            key={f.path}
            type="button"
            className={`ftab${current?.path === f.path ? ' on' : ''}`}
          ...
          >
            ...
            <span className="pl">+{f.insertions}</span>
            <span className="mn">−{f.deletions}</span>
          </button>
        ))}
      </div>
        ...
        {current && (
          <>
            ...
            {current.confidence === 'shared' ? (
              <span className="warn">
                <TriangleAlert size={11} aria-hidden="true" /> also changed by {current.alsoChangedBy.join(', ')}
              </span>
            ) : current.confidence === 'ambiguous' ? (
              <span className="warn">
                <TriangleAlert size={11} aria-hidden="true" /> another session was running — may not be this session's
                change
              </span>
            ) : current.touched ? (
